...
if raw_file_type != "delta":
    ...
    file_full_path = f"{raw_abfss_path}/{raw_connection_name}/{table_name}/version={version_number}/{load_action_text}/{datetime_folder_hierarchy}/{table_name}.{raw_file_type}"
...
# check schema exists
schema_exists = check_exists_delta_schema(schema_name=cleansed_connection_name)

# create schema, if required
if schema_exists == False:
    create_schema(container_name=cleansed_container_name, schema_name=cleansed_connection_name)

# COMMAND ----------

# check Delta Objects exist (import check functions)
# set Delta Table file path
location = set_delta_table_location(schema_name=cleansed_connection_name, table_name=table_name, abfss_path=cleansed_abfss_path)

# check Delta table exists
cleansed_table_path = set_table_path(schema_name =cleansed_connection_name, table_name =table_name)
...
if table_exists == False:
    ...
    create_table(container_name=cleansed_container_name, schema_name=cleansed_connection_name, table_name=table_name,location=location, partition_fields_sql=partition_fields_sql, columns_string=columns_string)
    table_created = True
    
    # get operations metrics 
    output = get_operation_metrics(schema_name=cleansed_connection_name, table_name=table_name, output=output)
...
target_delta = get_target_delta_table(schema_name = cleansed_connection_name, table_name=table_name)

write_to_delta_executor(write_mode=write_mode, target_df=target_delta, df=df, schema_name=cleansed_connection_name, table_name=table_name, pk_fields=pk_list, columns_list=total_column_list, partition_fields=partition_list)
