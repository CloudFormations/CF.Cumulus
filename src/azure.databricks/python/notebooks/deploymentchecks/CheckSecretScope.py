...
try:
    if any(scope.name == scope_name for scope in dbutils.secrets.listScopes()):
        ...
        raise ValueError(f"Secret scope '{scope_name}' does not exist. Please ensure this is created for successful integration between Databricks and Azure Key Vault.")
...
try:
    if len(dbutils.secrets.list(scope_name)) > 0:
        print(f"Secrets in scope '{scope_name}' are visible.")
    elif len(dbutils.secrets.list(scope_name)) == 0:
        raise ValueError(f"No secrets in scope '{scope_name}'. Please ensure secrets are created for successful integration between Databricks and Azure Key Vault.")
