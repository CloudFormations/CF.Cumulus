CREATE PROCEDURE [cumulus.control].[CheckForEmailAlerts]
	(
	@PipelineId INT
	)
AS
BEGIN
	...
	--get property
	SELECT
		@AlertingEnabled = [cumulus.control].[GetPropertyValueInternal]('UseFrameworkEmailAlerting');
		...
		BEGIN
			...
			IF EXISTS
				...
				SELECT 
					pal.AlertId
				FROM 
					[cumulus.control].CurrentExecution AS ce
					INNER JOIN [cumulus.control].AlertOutcomes AS ao
						ON ao.PipelineOutcomeStatus = ce.PipelineStatus
					INNER JOIN [cumulus.control].PipelineAlertLink AS pal
						ON pal.PipelineId = ce.PipelineId
					INNER JOIN [cumulus.control].Recipients AS r
