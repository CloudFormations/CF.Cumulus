CREATE PROCEDURE [cumulus.control].[CheckMetadataIntegrity]
	(
	@DebugMode BIT = 0,
...
BEGIN
	...
	*/
	...
	IF NOT EXISTS
		(
		SELECT 1 FROM [cumulus.control].[Stages] WHERE [Enabled] = 1
		)
		BEGIN
	...
	IF NOT EXISTS
		(
		SELECT 1 FROM [cumulus.control].[Pipelines] WHERE [Enabled] = 1
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
	...
	IF NOT EXISTS
		(
		SELECT * FROM [cumulus.control].[Tenants]
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
				( 
				4,
				'TenantId value is missing from the [cumulus.control].[Tenants] table.'
				)		
		END;
	...
	IF NOT EXISTS
		(
		SELECT * FROM [cumulus.control].[Subscriptions]
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
				( 
				5,
				'SubscriptionId value is missing from the [cumulus.control].[Subscriptions] table.'
				)		
		END;
	...
	IF NOT EXISTS
		(
		SELECT * FROM [cumulus.control].[CurrentProperties] WHERE [PropertyName] = 'OverideRestart'
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
	...
	IF EXISTS
		(
		SELECT * FROM [cumulus.control].[Orchestrators] WHERE [SubscriptionId] = '12345678-1234-1234-1234-012345678910'
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
	...
	IF EXISTS
		(
		SELECT * FROM [cumulus.control].[Subscriptions] WHERE [TenantId] = '12345678-1234-1234-1234-012345678910' AND [SubscriptionId] <> '12345678-1234-1234-1234-012345678910'
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
	...
	IF NOT EXISTS
		(
		SELECT * FROM [cumulus.control].[CurrentProperties] WHERE [PropertyName] = 'PipelineStatusCheckDuration'
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
	...
	IF NOT EXISTS
		(
		SELECT * FROM [cumulus.control].[CurrentProperties] WHERE [PropertyName] = 'UseFrameworkEmailAlerting'
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
	...
	IF (
		...
		BEGIN
			IF NOT EXISTS
				(
				SELECT * FROM [cumulus.control].[CurrentProperties] WHERE [PropertyName] = 'EmailAlertBodyTemplate'
				)
				BEGIN
					INSERT INTO @MetadataIntegrityIssues
					VALUES
	...
	IF EXISTS
		(
		SELECT * FROM [cumulus.control].[PipelineParameterDataSizes] WHERE [Size] > 9
		/*
		...
		*/
		...
		BEGIN
			...
			VALUES
				( 
				13,
				'The pipeline parameters entered exceed the Azure Function request body maximum of 10MB. Query view [cumulus.control].[PipelineParameterDataSizes] for details.'
				)	
		END;
	...
	IF NOT EXISTS
		(
		SELECT * FROM [cumulus.control].[CurrentProperties] WHERE [PropertyName] = 'FailureHandling'
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
	...
	BEGIN
		...
		SELECT 
			pd.[DependencyId]
		FROM 
			[cumulus.control].[PipelineDependencies] pd
	...
	IF NOT EXISTS
		(
		SELECT * FROM [cumulus.control].[CurrentProperties] WHERE [PropertyName] = 'UseExecutionBatches'
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
	...
	IF NOT EXISTS
		(
		SELECT * FROM [cumulus.control].[CurrentProperties] WHERE [PropertyName] = 'FrameworkFactoryResourceGroup'
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
	...
	IF NOT EXISTS
		(
		SELECT * FROM [cumulus.control].[CurrentProperties] WHERE [PropertyName] = 'PreviousPipelineRunsQueryRange'
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
	...
	IF ([cumulus.control].[GetPropertyValueInternal]('UseExecutionBatches')) = '1'
		BEGIN			
			...
			IF EXISTS
				(
				SELECT 1 FROM [cumulus.control].[Batches] WHERE [BatchId] = @BatchId AND [Enabled] = 0
				)
				BEGIN
					INSERT INTO @MetadataIntegrityIssues
					VALUES
			...
			IF NOT EXISTS
				(
				SELECT 1 FROM [cumulus.control].[BatchStageLink] WHERE [BatchId] = @BatchId
				)
				BEGIN
					INSERT INTO @MetadataIntegrityIssues
					VALUES
						( 
						23,
						'The requested execution batch does not have any linked execution stages. See table [cumulus.control].[BatchStageLink] for details.'
						)
				END;
			...
			IF EXISTS
				...
				SELECT
					*
				FROM
					[cumulus.control].[CurrentExecution] c
					LEFT OUTER JOIN [cumulus.control].[BatchExecution] b
				...
				BEGIN
					...
					VALUES
						( 
						24,
						'Execution records exist in the [cumulus.control].[CurrentExecution] table that do not have a record in [cumulus.control].[BatchExecution] table. Has batch excutions been enabed after an incomplete none batch run?'
						)
				END;			
	...
	IF EXISTS
		(
		SELECT 1 FROM [cumulus.control].[CurrentExecution] WHERE [PipelineStatus] = 'InvalidPipelineNameError'
		)
		BEGIN
			UPDATE
				ce
			...
			FROM
				[cumulus.control].[CurrentExecution] ce
	...
	--Check 26:
	IF (SELECT COUNT(0) FROM [cumulus.control].[Orchestrators] WHERE [IsFrameworkOrchestrator] = 1) > 1
	BEGIN
		INSERT INTO @MetadataIntegrityIssues
		VALUES
			( 
			26,
			'There is more than one FrameworkOrchestrator set in the table [cumulus.control].[Orchestrators]. Only one is supported.'
			)		
	END
	...
	IF NOT EXISTS
		(
		SELECT 1 FROM [cumulus.control].[Orchestrators] WHERE [IsFrameworkOrchestrator] = 1
		)
		BEGIN
			INSERT INTO @MetadataIntegrityIssues
			VALUES
				( 
				27,
				'A FrameworkOrchestrator has not been set in the table [cumulus.control].[Orchestrators]. Only one is supported.'
				)		
		END
	...
	*/
	...
	IF EXISTS
		...
		AND @DebugMode = 0
		BEGIN
			SET @ErrorDetails = 'Metadata integrity checks failed. Run EXEC [cumulus.control].[CheckMetadataIntegrity] @DebugMode = 1; for details.'
