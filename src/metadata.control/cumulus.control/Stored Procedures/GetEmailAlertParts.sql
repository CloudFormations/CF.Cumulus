CREATE PROCEDURE [cumulus.control].[GetEmailAlertParts]
	(
	@PipelineId INT
	)
AS
BEGIN
	...
	FROM
		...
		INNER JOIN [cumulus.control].[AlertOutcomes] ao
	...
	IF @EmailBody IS NULL
		SET @EmailBody = 'Internal error. Failed to create profwk email alert body. Execute procedure [cumulus.control].[GetEmailAlertParts] with pipeline Id: ' + CAST(@PipelineId AS VARCHAR(30)) + ' to debug.';
