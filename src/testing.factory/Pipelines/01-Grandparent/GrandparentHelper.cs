...
WHERE [PropertyName] = 'UseExecutionBatches'");
        ...
        {
            ExecuteNonQuery("UPDATE [cumulus.control].[Properties] SET [PropertyValue] = 'Simple' WHERE [PropertyName] = 'FailureHandling'");
            return this;
        }
        public GrandparentHelper WithOneWorkerPipelineEnabled()
        {
            ExecuteNonQuery("UPDATE [cumulus.control].[Pipelines] SET [Enabled] = 0 WHERE [PipelineId] > 1");
            return this;
        }
