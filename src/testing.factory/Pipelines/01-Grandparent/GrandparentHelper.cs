...
WHERE [PropertyName] = 'UseExecutionBatches'");
        ...
        {
            ExecuteNonQuery("UPDATE [control].[Properties] SET [PropertyValue] = 'Simple' WHERE [PropertyName] = 'FailureHandling'");
            return this;
        }
        public GrandparentHelper WithOneWorkerPipelineEnabled()
        {
            ExecuteNonQuery("UPDATE [control].[Pipelines] SET [Enabled] = 0 WHERE [PipelineId] > 1");
            return this;
        }
